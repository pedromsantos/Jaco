...
{
	private readonly Dictionary<Interval, Pitch> intervalsToPitches = new();
	...
	{
		return new IntervalsToPitches(intervalsToPitches.Select((keyValue) => Equals(keyValue.Key, interval) ? new KeyValuePair<Interval, Pitch>(keyValue.Key, pitch) : keyValue));
	}
	...
	{
		return intervalsToPitches.First(x => Equals(x.Value, pitch)).Key;
	}
